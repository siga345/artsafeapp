...
- `/` не редиректит всегда в одну точку. Он читает сессию, `UserOnboardingState` и progression-сигналы.
- Если пользователь не залогинен, он уходит на `/signin`.
- Если это новый пользователь без завершённой анкеты, он уходит на `/welcome`.
- Если анкета уже завершена, но first-time guide ещё не закрыт, пользователь всё равно приходит в `/today`, а дальше guide-layer сам ведёт его по шагам.
...
- first-time guide живёт глобально и переживает переходы между маршрутами.
...
Отдельно есть регистрация на `/signup`:

1. Форма идёт в `src/app/api/auth/register/route.ts`.
2. Там создаётся `User` и стартовая запись `UserOnboardingState`.
3. После этого пользователь автоматически логинится и попадает в `/welcome`.
...
Новый слой входа построен вокруг:

- `src/lib/entry-flow.ts`
- `src/app/api/onboarding/route.ts`
- `src/app/api/onboarding/survey/route.ts`
- `src/components/onboarding/entry-guide-provider.tsx`
- `src/app/welcome/page.tsx`

То есть auth теперь заканчивается не просто входом в систему. Для новых пользователей дальше начинается product flow:

1. visual survey;
2. guide по ключевым маршрутам;
3. возврат на `/today` с приоритетом на `Мир артиста`.
...
`UserOnboardingState` больше не только про dismiss-state. Сейчас это полноценный first-time state:
...
- плюс старый `dismissedAt` для home-onboarding
...
- `src/app/api/id/journey/route.ts`
...
Сейчас важно различать два режима:

- `/id` — привычный preview `Мира артиста` + блок `SAFE ID`
- `/id/edit` — расширенный редактор

То есть пользовательский read-surface и edit-surface разведены.
...
А первая инициализация части этих данных теперь может приходить не только из старого ID-onboarding, но и из visual survey на `/welcome`.
...
- Signup/onboarding: `src/app/signup/page.tsx`, `src/app/welcome/page.tsx`, `src/lib/entry-flow.ts`, `src/components/onboarding/entry-guide-provider.tsx`
