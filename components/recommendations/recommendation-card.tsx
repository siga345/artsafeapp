...
const recommendationKindLabels: Record<RecommendationCardData["kind"], string> = {
  NEXT_STEP: "Следующий шаг для текущего этапа",
  DIAGNOSTIC: "Проверка системной готовности",
  GOAL_ACTION: "Шаг, который укрепит карьерную систему",
  TODAY_FOCUS: "Фокус текущего цикла",
  LEARN_CONTEXT: "Зона, которую стоит усилить"
};
...
export function RecommendationCard({ recommendation, className, meta, onAction, hideFutureAiSlot = false }: Props) {
  ...
  const recommendationLensLabel = recommendationKindLabels[recommendation.kind];
  ...
  return (
    <section className={cn("rounded-2xl border border-brand-border bg-white/85 p-4", className)}>
      ...
      <p className="mt-3 text-[11px] uppercase tracking-[0.16em] text-brand-muted">{recommendationLensLabel}</p>
      ...
      {recommendation.reason ? <p className="mt-2 text-sm text-brand-muted">Почему сейчас: {recommendation.reason}</p> : null}
