...
import { getAbsoluteStoragePath, storageProvider } from "@/lib/storage";
...
export const GET = withApiHandler(async (request: Request, { params }: { params: { id: string } }) => {
  ...
  if (!demo.audioUrl) {
    throw apiError(404, "Audio file not found");
  }
  ...
  const absolutePath = getAbsoluteStoragePath(demo.audioUrl);
