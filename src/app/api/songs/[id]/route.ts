...
export const DELETE = withApiHandler(async (_: Request, { params }: { params: { id: string } }) => {
  ...
  const demoStorageKeys = [...new Set(track.demos.map((demo) => demo.audioUrl).filter(Boolean))];
  ...
  await Promise.allSettled(demoStorageKeys.map((storageKey) => storageProvider.deleteFile(storageKey)));
