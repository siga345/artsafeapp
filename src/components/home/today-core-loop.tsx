...
import { getPhase1PathStageName } from "@/lib/path-stages";
...
export function TodayCoreLoop({ checkIn, dayLoop, onRefresh, className }: Props) {
  ...
  const selectedTrackStageName = getPhase1PathStageName({ order: selectedTrack?.pathStage?.id, name: selectedTrack?.pathStage?.name });
  const focusedTrackStageName = getPhase1PathStageName({ order: focusedTrack?.pathStage?.id, name: focusedTrack?.pathStage?.name });
  ...
  async function saveCheckIn() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await readApiErrorMessage(response, "Не удалось сохранить чек-ин."));
      }
      await onRefresh();
      toast.success("Состояние дня сохранено.");
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Не удалось сохранить чек-ин.");
    } finally {
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Card className="space-y-4">
          <CardHeader className="mb-0">
            ...
            <CardDescription>Состояние дня, один рабочий объект и один следующий шаг.</CardDescription>
          </CardHeader>
          ...
          {selectedTrack ? (
            <div className="rounded-2xl border border-brand-border bg-white/80 p-4">
              ...
              <p className="mt-1 text-xs text-brand-muted">
                ...
                {selectedTrackStageName ? ` • ${selectedTrackStageName}` : ""}
              </p>
              ...
              {selectedTrack.activeNextStep ? (
                <RecommendationCard
                  ...
                  recommendation={buildTodayNextStepRecommendation(selectedTrack.activeNextStep, "Текущий шаг", selectedTrack.id)}
                />
              ) : (
                <div className="mt-3 space-y-2">
                  <Input
                    ...
                    placeholder="Следующий шаг по объекту"
                  />
                  <Textarea
            ...
            <div className="rounded-2xl border border-dashed border-brand-border bg-white/70 p-4 text-sm text-brand-muted">
              Пока нет выбранного объекта. Открой раздел «Песни» и создай хотя бы один рабочий слой.
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            <Button disabled={savingCheckIn} onClick={() => void saveCheckIn()}>
              {savingCheckIn ? "Сохраняю..." : "Сохранить чек-ин"}
            </Button>
        ...
        <Card className="space-y-4">
          <CardHeader className="mb-0">
            ...
            <CardDescription>Текущий объект и его активный шаг прямо из экрана пути.</CardDescription>
          </CardHeader>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-brand-muted">
                      ...
                      {focusedTrackStageName ? ` • ${focusedTrackStageName}` : ""}
                    </p>
                  </div>
              ...
              <div className="rounded-2xl border border-brand-border bg-[#f7fbf2] p-4">
                ...
                {focusedNextStep ? (
                  ...
                  <p className="mt-2 text-sm text-brand-muted">У этого объекта ещё нет активного следующего шага.</p>
                )}
              </div>

              <Link href={`/songs/${focusedTrack.id}`}>
                <Button className="w-full justify-between">
                  Открыть объект в Песнях
          ...
          {dayLoop?.wrapUp?.nextStep ? (
            <div className="rounded-2xl border border-emerald-300/60 bg-[#edf8f0] p-4 text-[#2c6a40]">
              ...
              <RecommendationCard
                ...
                recommendation={buildTodayNextStepRecommendation(dayLoop.wrapUp.nextStep, "Следующий шаг на завтра", dayLoop.wrapUp.trackId)}
              />
            </div>
          ) : null}
        </Card>
