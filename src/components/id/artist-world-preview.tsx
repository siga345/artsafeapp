...
import { Link2, ListMusic, MapPin, Shapes, UserRound, Users } from "lucide-react";
...
export function ArtistWorldPreview(props: {
  ...
  return (
    <div
      className={`overflow-hidden rounded-[28px] border ${theme.panel} p-3 shadow-[0_18px_36px_rgba(16,21,19,0.14)] backdrop-blur-sm sm:p-4 ${theme.shell}`}
      style={backgroundStyle}
    >
      <div className="space-y-4">
        <section className={`rounded-[24px] border p-3 sm:p-4 ${theme.card}`}>
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-current/10 bg-white/10 sm:h-16 sm:w-16">
              {props.avatarUrl ? (
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[11px] uppercase tracking-[0.18em] opacity-70">Мир артиста</p>
              <h2 className="mt-1 text-2xl font-semibold tracking-tight">{props.nickname || "Новый артист"}</h2>

              <div className="mt-2 flex flex-wrap items-center gap-3">
                <span className={`inline-flex max-w-full items-center gap-1 text-xs ${theme.muted}`}>
              ...
              <p className={`mt-3 max-w-2xl text-sm leading-7 ${theme.muted}`}>
                {renderText(
                  ...
                  props.artistWorld.mission ?? "Добавь базовые ответы, чтобы мир артиста заговорил."
                )}
              </p>
              ...
              {props.bandlink ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`mt-3 inline-flex w-full items-center justify-center gap-2 rounded-full border px-3 py-2 text-xs sm:w-auto ${theme.accent}`}
                >
        ...
        <section className={`rounded-[24px] border p-3 sm:p-4 ${theme.card}`}>
          ...
          <div className="mt-3 grid gap-4 sm:grid-cols-2">
            <div>
              <p className="text-sm font-medium">Смысл и опора</p>
            ...
            <div>
              <p className="text-sm font-medium">Миссия и движение</p>
              <p className={`mt-1 text-sm leading-6 ${theme.muted}`}>
        ...
        <section className={`rounded-[24px] border p-3 sm:p-4 ${theme.card}`}>
          ...
          {(props.artistWorld.visualDirection ||
            ...
            <div className="mt-4 rounded-2xl border border-current/10 bg-white/10 p-4">
              <p className="text-xs uppercase tracking-[0.12em] opacity-70">Образ и персонаж</p>
        ...
        <section className={`rounded-[24px] border p-3 sm:p-4 ${theme.card}`}>
          ...
          {props.artistWorld.playlistUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className={`mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-medium transition-opacity hover:opacity-80 sm:w-auto ${theme.accent}`}
            >
            ...
            <p className={`mt-3 text-sm ${theme.muted}`}>
              Плейлист пока не добавлен. Он нужен как опора для направления, а не как замена смыслу или образу.
            </p>
          )}
        </section>

        {(props.artistWorld.currentFocusTitle || props.artistWorld.currentFocusDetail || props.artistWorld.seekingSupportDetail) ? (
          <section className={`rounded-[24px] border p-3 sm:p-4 ${theme.card}`}>
            <p className="text-[11px] uppercase tracking-[0.18em] opacity-70">Фокус сейчас</p>
            {props.artistWorld.currentFocusTitle ? <p className="mt-2 text-sm font-medium">{props.artistWorld.currentFocusTitle}</p> : null}
            {props.artistWorld.currentFocusDetail ? <p className={`mt-2 text-sm leading-6 ${theme.muted}`}>{props.artistWorld.currentFocusDetail}</p> : null}
            {props.artistWorld.seekingSupportDetail ? (
              <p className={`mt-2 text-xs leading-5 ${theme.muted}`}>{props.artistWorld.seekingSupportDetail}</p>
            ) : null}
          </section>
        ) : null}
      </div>
