...
function buildArtistWorldDiagnostic(profile: ArtistWorldInput | null): DiagnosticItem {
  ...
  if (textCoreCount === 0) {
    return {
      ...
      message: "Пока не собрана базовая текстовая основа мира артиста, поэтому стратегия и задачи висят без опоры. Карьера артиста начинается со смысла, образа и направления, а не только с песен.",
      recommendation: buildSystemRecommendation({
        key: "home:diagnostic:ARTIST_WORLD",
        kind: "DIAGNOSTIC",
        title: "Заполнить мир артиста",
        text: "Пока не собрана базовая текстовая основа мира артиста, поэтому стратегия и задачи висят без опоры. Карьера артиста начинается со смысла, образа и направления, а не только с песен.",
        href: "/id"
      })
    };
  }
  ...
  if (!textCoreReady) {
    return {
      ...
      message: "Дособери миссию, самоописание и тематическое ядро, чтобы мир артиста начал читаться цельно. Песни сами по себе еще не складываются в карьеру без образа и смысла.",
      recommendation: buildSystemRecommendation({
        key: "home:diagnostic:ARTIST_WORLD",
        kind: "DIAGNOSTIC",
        title: "Дописать ядро мира",
        text: "Дособери миссию, самоописание и тематическое ядро, чтобы мир артиста начал читаться цельно. Песни сами по себе еще не складываются в карьеру без образа и смысла.",
        href: "/id"
      })
    };
  }
  ...
  if (!hasVisual) {
    return {
      ...
      message: "Базовая история артиста уже есть, но без визуальных референсов мир пока не складывается в цельный образ. Карьера артиста требует не только текста, но и читаемого персонажа.",
      recommendation: buildSystemRecommendation({
        key: "home:diagnostic:ARTIST_WORLD",
        kind: "DIAGNOSTIC",
        title: "Усилить образ артиста",
        text: "Базовая история артиста уже есть, но без визуальных референсов мир пока не складывается в цельный образ. Карьера артиста требует не только текста, но и читаемого персонажа.",
        href: "/id"
      })
    };
  }
  ...
  return {
    ...
    message: "Текстовая основа и визуальные опоры уже собраны и могут работать как фундамент для стратегии. Это уже не просто профиль, а внутренняя система артиста.",
    recommendation: buildSystemRecommendation({
      key: "home:diagnostic:ARTIST_WORLD",
      kind: "DIAGNOSTIC",
      title: "Открыть мир артиста",
      text: "Текстовая основа и визуальные опоры уже собраны и могут работать как фундамент для стратегии. Это уже не просто профиль, а внутренняя система артиста.",
      href: "/id"
    })
  };
}
