...
describe("parseJsonBody", () => {
  ...
  it("throws when body is not valid JSON", async () => {
    ...
    await expect(parseJsonBody(req, testSchema)).rejects.toMatchObject({
      status: 400,
      message: "Invalid request body",
    });
  });
  ...
  it("lets withApiHandler return 400 for malformed JSON", async () => {
    const handler = withApiHandler(async (request: Request) => {
      await parseJsonBody(request, testSchema);
