...
function getRedactedSupportProfile(): CommunityProfileDto["supportProfile"] {
  return {
...
export async function getCommunityProfile(db: DbClient, viewerUserId: string, safeId: string): Promise<CommunityProfileDto> {
  ...
  const canViewPrivateContext = profile.id === viewerUserId;
  ...
  const [friendRows, friendsCount, visibleAchievementRows, visibleAchievementIds, goingEventsCount, derivedFocus] = await Promise.all([
    ...
    canViewPrivateContext ? buildDerivedFocus(db, profile.id) : Promise.resolve(null),
  ]);
  ...
  return {
    ...
    supportProfile: canViewPrivateContext
      ...
      : getRedactedSupportProfile(),
    derivedFocus: canViewPrivateContext && derivedFocus
      ? {
          ...derivedFocus,
          supportNeedTypes:
...
export async function resolveOwnedCommunityPostRefs(
  db: DbClient,
  viewerUserId: string,
  input: {
    trackId?: string | null;
  ...
  const requestedTrackId = input.trackId?.trim() || null;
  const requestedDemoId = input.demoId?.trim() || null;

  if (!requestedTrackId && !requestedDemoId) {
    return {
      trackId: null,
      demoId: null
    };
  ...
  if (requestedDemoId) {
    ...
    if (!demo) {
      throw apiError(404, "Демо не найдено.");
    }

    if (requestedTrackId && demo.trackId !== requestedTrackId) {
      throw apiError(400, "Демо не принадлежит выбранному треку.");
    }
...
export async function createProgressCommunityPost(
  ...
  const refs = await resolveOwnedCommunityPostRefs(db, viewerUserId, input);
  return createTypedCommunityPost(db, viewerUserId, {
    kind: CommunityPostKind.PROGRESS,
    title: input.title,
    text: input.text,
    trackId: refs.trackId,
    demoId: refs.demoId
  });
}
