...
describe("buildDiagnostics — ARTIST_WORLD", () => {
  it("returns MISSING state when identityProfile is null", () => {
    ...
    expect(world?.title).toBe("Мир артиста не собран");
    expect(world?.message).toContain("не только с песен");
    expect(world?.recommendation.title).toBe("Заполнить мир артиста");
  });
  ...
  it("returns WEAK when text core is filled only partially", () => {
    ...
    expect(world?.recommendation.title).toBe("Дописать ядро мира");
  });
  ...
  it("returns IN_PROGRESS when text core is complete but visual boards are empty", () => {
    ...
    expect(world?.recommendation.title).toBe("Усилить образ артиста");
  });
  ...
  it("returns STRONG when text core is complete and visual boards have a source url", () => {
    ...
    expect(world?.recommendation.title).toBe("Открыть мир артиста");
  });
});
