...
export function serializeDemoPlayback(demo: { id: string; audioUrl: string | null }) {
  const hasAudio = Boolean(demo.audioUrl?.trim());
  return {
    hasAudio,
    audioFileName: hasAudio ? getStorageDisplayFilename(demo.audioUrl!) : null,
    audioStreamUrl: hasAudio ? `/api/audio-clips/${demo.id}/stream` : null
  };
}
...
export function serializeTrackListItem(track: TrackListRecord, options?: TrackSerializationOptions) {
    ...
    ? {
        ...
        masterDemo: track.distributionRequest.masterDemo
          ? {
              ...
              ...serializeDemoPlayback(track.distributionRequest.masterDemo),
  ...
  const serializedReleaseDemo = releaseDemo
    ? {
        ...
        ...serializeDemoPlayback(releaseDemo),
  ...
  return {
    ...
    primaryDemo: track.primaryDemo
      ? {
          id: track.primaryDemo.id,
          duration: track.primaryDemo.duration,
          versionType: track.primaryDemo.versionType,
          ...serializeDemoPlayback(track.primaryDemo),
    ...
    latestDemo: latestDemo
      ? {
          id: latestDemo.id,
          ...serializeDemoPlayback(latestDemo),
  ...
  return {
    ...
    primaryDemo: track.primaryDemo
      ? {
          id: track.primaryDemo.id,
          duration: track.primaryDemo.duration,
          textNote: track.primaryDemo.textNote ?? null,
          versionType: track.primaryDemo.versionType,
          ...serializeDemoPlayback(track.primaryDemo),
          detectedBpm: track.primaryDemo.detectedBpm ?? null,
          detectedBpmConfidence: track.primaryDemo.detectedBpmConfidence ?? null,
          detectedKeyRoot: track.primaryDemo.detectedKeyRoot ?? null,
          detectedKeyMode: track.primaryDemo.detectedKeyMode ?? null,
          detectedKeyConfidence: track.primaryDemo.detectedKeyConfidence ?? null,
          detectedAnalysisSource: track.primaryDemo.detectedAnalysisSource ?? null,
          detectedAnalysisVersion: track.primaryDemo.detectedAnalysisVersion ?? null,
          detectedAt: toIso(track.primaryDemo.detectedAt),
          sortIndex: track.primaryDemo.sortIndex,
          createdAt: track.primaryDemo.createdAt.toISOString(),
          releaseDate: track.primaryDemo.releaseDate ? track.primaryDemo.releaseDate.toISOString().slice(0, 10) : null
        }
      : null,
    ...
    demos: track.demos.map((demo) => ({
      ...
      detectedBpmConfidence: demo.detectedBpmConfidence ?? null,
      detectedKeyRoot: demo.detectedKeyRoot ?? null,
      detectedKeyMode: demo.detectedKeyMode ?? null,
      detectedKeyConfidence: demo.detectedKeyConfidence ?? null,
      detectedAnalysisSource: demo.detectedAnalysisSource ?? null,
      detectedAnalysisVersion: demo.detectedAnalysisVersion ?? null,
